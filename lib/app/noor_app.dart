...
class NoorApp extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final router = GoRouter(routes: [
      ...
      GoRoute(path: '/challenge/:id', builder: (_, s) => ChallengeScreen(id: int.parse(s.pathParameters['id']!))),
    ...
    return MaterialApp.router(
      ...
      builder: (context, child) => Directionality(textDirection: state.locale.languageCode == 'ar' ? TextDirection.rtl : TextDirection.ltr, child: child!),
      routerConfig: router,
    );
  }
}
...
class _SplashScreenState extends State<SplashScreen> {
  @override void initState() { super.initState(); Future.delayed(const Duration(milliseconds: 900), () { if (mounted) context.go('/language'); }); }
  @override Widget build(BuildContext context) { final l = AppLocalizations.of(context)!; return Scaffold(body: Container(alignment: Alignment.center, decoration: const BoxDecoration(gradient: LinearGradient(colors: [Color(0xFFFFB45E), Color(0xFFFFF1CF)])), child: Column(mainAxisSize: MainAxisSize.min, children: [const Text('🏮', style: TextStyle(fontSize: 96)), Text(l.arabicTitle, style: Theme.of(context).textTheme.headlineMedium), Text(l.appTitle, style: Theme.of(context).textTheme.titleLarge), const SizedBox(height: 16), Text(l.splashTagline)]))); }
}
